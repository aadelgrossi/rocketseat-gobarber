...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (!isProvider) {
      return res
        .status(401)
        .json({ error: 'ID given does not match a provider' });
    }
    ...
    if (checkAvailability) {
      return res
        .status(400)
        .json({ error: 'Appointment date with provider is already taken' });
    }
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      // if appointment time is less than 2 hours away, do not cancel
      return res.status(401).json({
